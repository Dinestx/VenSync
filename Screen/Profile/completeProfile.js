import React, { useState, useEffect } from 'react';
import { View, Text, TextInput, TouchableOpacity, Alert } from 'react-native';
import { ScreenH, ScreenW, BackIcon } from '../Component/exportAsset';
import { useTheme } from '../Component/theme'
import { useNavigation } from '@react-navigation/native';
import CheckBox from '@react-native-community/checkbox';
import Lottie from 'lottie-react-native';
import axios from 'axios';
import AsyncStorage from '@react-native-async-storage/async-storage';



export default function CompleteProfile() {

    const theme = useTheme();
    const navigation = useNavigation();
    const [isnameFocused, setNameFocused] = useState(false);
    const [isDisFocused, setDisFocused] = useState(false);
    const [isAddFocused, setAddFocused] = useState(false);

    const [fullName, setFullName] = useState('');
    const [phoneNumber, setPhoneNumber] = useState('');
    const [rawPhoneNumber, setRawPhoneNumber] = useState('');
    const [homeAddress, setHomeAddress] = useState('');
    const [isselPrivacy, setSelPrivacy] = useState(false);
    const [isselTerms, setSelTerms] = useState(false);




    const formatPhoneNumber = (text) => {
        // Remove non-digit characters
        const cleaned = text.replace(/\D+/g, '');

        // Apply formatting
        const formatted = cleaned
            .replace(/(\d{4})(\d{3})(\d{0,3})/, (_, p1, p2, p3) => {
                return `${p1}-${p2}-${p3}`.replace(/-$/, '');
            });

        setPhoneNumber(formatted);
        setRawPhoneNumber(cleaned);
    };

    const updateProfile = async () => {
        if (!fullName.trim()) {
            // Alert the user if the full name is empty
            Alert.alert('Required', 'Full name cannot be empty');
            return; // Exit the function if validation fails
        }

        // Prepare the user data
        const user = {
            name: fullName,
            phone: rawPhoneNumber,
            address: homeAddress
        };

        // Get the token from AsyncStorage
        const token = await AsyncStorage.getItem("token");

        if (!token) {
            // Check if token exists
            Alert.alert('Authentication Error', 'Token not found');
            return;
        }

        try {
            // Send the profile update request to the server
            const response = await axios.post(
                'https://vensync-se39.onrender.com/api/complete-profile',
                user,
                {
                    headers: { 
                        "Authorization": `Bearer ${token}`, // Correct header name
                    }
                }
            ).then(res=> console.log(res.data)) ;
        
            
            console.log(response.data); // Log the response data or handle it as needed

            // Optionally show a success message or navigate to another screen after successful update
            Alert.alert('Profile Updated', 'Your profile has been updated successfully!');
        } catch (error) {
            // Catch and handle any errors from the API request
            console.error(error);
            Alert.alert('Error', 'Failed to update profile. Please try again.');
        }
    };



    return (
        <View style={{ backgroundColor: theme.bgColor, flex: 1, flexDirection: 'column', paddingHorizontal: 25, }}>
            {/* App Bar */}
            <View style={{ paddingTop: ScreenH * 0.055, }}>
                <BackIcon style={{ paddingVertical: 18, color: theme.primaryText, backgroundColor: theme.SkipColor, borderRadius: 18 }} onPress={() => navigation.goBack()} />
            </View>

            {/* Text ============================================= */}
            <View style={{ paddingVertical: ScreenH * 0.025, paddingHorizontal: 5 }}>
                <Text style={{ fontSize: ScreenW * 0.065, fontWeight: '600', color: theme.primaryText }}>Complete your profile</Text>
                <Text style={{ fontSize: ScreenW * 0.035, color: theme.primaryText }}>View promotions that are displayed in membership marketplace.</Text>
            </View>


            <View style={{ gap: 15 }}>
                {/* Name =========================================== */}
                <View style={{
                    height: ScreenH * 0.057,
                    borderRadius: 12,
                    backgroundColor: theme.textInput,
                    borderWidth: 2,
                    borderColor: isnameFocused ? theme.nameActive : '#3d305e',
                }}
                    onFocus={() => setNameFocused(true)} // Set focus state to true
                    onBlur={() => setNameFocused(false)} // Set focus state to false
                >
                    <TextInput
                        style={{
                            color: theme.placeholder,
                            fontSize: ScreenW * 0.04,
                            flex: 1,
                            marginLeft: 10,
                            fontWeight: 'bold',
                            alignItems: 'center',
                            letterSpacing: 1,
                        }}
                        value={fullName}
                        onChange={setFullName}
                        placeholder="Full Name"
                        keyboardType="default"
                        placeholderTextColor={isnameFocused ? '#3d305e' : theme.nameActive}

                    />
                </View>



                {/* Mobile Number ========================================== */}
                <View style={{
                    flexDirection: 'row',
                    height: ScreenH * 0.057,
                    borderRadius: 15,
                    backgroundColor: theme.textInput,
                    borderWidth: 2,
                    borderColor: isDisFocused ? theme.nameActive : '#3d305e',
                    alignItems: 'center',
                    paddingHorizontal: 10
                }}
                    onFocus={() => setDisFocused(true)}
                    onBlur={() => setDisFocused(false)}
                >
                    <Text style={{ color: !isDisFocused ? theme.nameActive : 'white', fontSize: ScreenW * 0.045, fontWeight: 'bold', alignItems: 'center' }}>+91</Text>
                    <TextInput
                        style={{
                            color: theme.placeholder,
                            fontSize: ScreenW * 0.045,
                            flex: 1,
                            marginLeft: 0,
                            fontWeight: 'bold',
                            alignItems: 'center',
                            letterSpacing: 7,
                        }}
                        placeholder="xxx-xxx-xxxx"
                        placeholderText
                        keyboardType="numeric"
                        placeholderTextColor={isDisFocused ? '#3d305e' : theme.nameActive}
                        value={phoneNumber}
                        onChangeText={formatPhoneNumber}
                        maxLength={12}
                    />
                </View>

                {/* Home Address =========================================== */}
                <View style={{
                    height: ScreenH * 0.15,
                    borderRadius: 12,
                    backgroundColor: theme.textInput,
                    borderWidth: 2,
                    borderColor: isAddFocused ? theme.nameActive : '#3d305e',
                }}
                    onFocus={() => setAddFocused(true)} // Set focus state to true
                    onBlur={() => setAddFocused(false)} // Set focus state to false
                >
                    <TextInput
                        style={{
                            color: theme.placeholder,
                            fontSize: ScreenW * 0.04,
                            flex: 1,
                            marginLeft: 10,
                            fontWeight: 'bold',
                            letterSpacing: 1,
                        }}
                        value={homeAddress}
                        onChange={setHomeAddress}
                        placeholder="Full Name"
                        keyboardType="default"
                        placeholderTextColor={isAddFocused ? '#3d305e' : theme.nameActive}
                    />
                </View>

                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 7 }}>
                    <CheckBox
                        value={isselPrivacy}
                        onValueChange={setSelPrivacy}
                        onCheckColor="#6F763F"
                        style={{ alignSelf: 'center' }}
                    />
                    <Text style={{ fontSize: 16, color: theme.primaryText }}>I read and agree to <Text style={{ fontSize: 16, fontWeight: 'bold', color: theme.nameActive }}>Privacy Policy</Text></Text>
                    <Text>I read and agree to Terms and Condition</Text>
                </View>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 7 }}>
                    <CheckBox
                        value={isselTerms} // Bind checkbox value to the state
                        onValueChange={setSelTerms} // Set function to update the state
                        onCheckColor="#6F763F" // Customize checkmark color
                        style={{ alignSelf: 'center' }}
                    />
                    <Text style={{ fontSize: 16, color: theme.primaryText }}>I read and agree to <Text style={{ fontSize: 16, fontWeight: 'bold', color: theme.nameActive }}>Privacy Policy</Text></Text>
                    <Text>I read and agree to Terms and Condition</Text>
                </View>


            </View>

            {/* Continue Button ===================================== */}
            <View style={{ flex: 1, justifyContent: 'flex-end', alignItems: 'center', paddingBottom: ScreenH * 0.055, marginTop: ScreenH * 0.072 }}>
                <TouchableOpacity
                    style={{
                        flexDirection: 'row',
                        width: ScreenW * 0.65,
                        backgroundColor: theme.button,
                        height: 50,
                        justifyContent: 'center',
                        alignItems: 'center',
                        borderRadius: 18,
                    }}
                    onPress={updateProfile}

                >
                    <Text style={{ textAlign: 'center', color: 'white', fontWeight: '600' }}>Raise Complaint</Text>
                </TouchableOpacity>
            </View>
        </View>
    )
}

